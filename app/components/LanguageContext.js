...
export function LanguageProvider({ children }) {
  ...
  useEffect(() => {
    const savedLang = normalizeLanguage(localStorage.getItem(LOCAL_STORAGE_KEY));
  ...
  const translateBatch = useCallback(async (values) => {
  ...
  const contextValue = useMemo(() => ({
    ...
    translateBatch,
    translateText,
  }), [changeLanguage, language, resolvedLanguage, t, translateBatch, translateText]);
...
export function TranslatedText({ children, className = '' }) {
  ...
  return (
    <span className={`${language !== 'en' ? 'font-semibold uppercase tracking-wide' : ''} ${className}`}>
      {children}
    </span>
  );
}
